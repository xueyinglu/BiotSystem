...
void BiotSystem::assemble_system_pressure()
{
    ...
    double prev_timestep_mean_stress(n_q_points);
    double prev_fs_mean_stress(n_q_points);
    ...
    for (; cell != endc; ++cell, ++cell_displacement)
    {
        ...
        for (unsigned int q = 0; q < n_q_points; q++)
        {
            ...
            prev_timestep_mean_stress = K_b * (prev_timestep_sol_grad_u_values[q][0][0] + prev_timestep_sol_grad_u_values[q][1][1]) - biot_alpha * prev_timestep_sol_pressure_values[q];
            ...
            for (unsigned int i = 0; i < dofs_per_cell; i++)
            {
                ...
                // prev time step 
                cell_rhs(i) += 
                (fe_value.shape_value(i, q) * 
                prev_timestep_sol_pressure_values[q] * 
                fe_value.JxW(q) / del_t);
