...
void BiotSystem::calc_a_posteriori_indicators_u()
{
            ...
            //if (! cell -> at_boundary(face_no))
            {
                ...
                if (cell->face(face_no)->has_children())
                {
                         ...
                         ++subface_no)
                    {
                        ...
                        vector<double> face_p_values(fe_subface_p.n_quadrature_points);
                ...
                else
                {
                    ...
                    vector<double> face_p_values(fe_face_p.n_quadrature_points);
                    vector<double> neighbor_p_values(fe_subface_p.n_quadrature_points);
                    vector<double> prev_timestep_face_p_values(fe_face_p.n_quadrature_points);
                    vector<double> prev_timestep_neighbor_p_values(fe_subface_p.n_quadrature_points);
    ...
    for (; cell != endc; ++cell, ++cell_u, ++cell_output)
    {
        ...
        {
            Tensor<1, dim> dum3;
            dum3[0] = (2 * mu_values[q] + lambda_values[q]) * (hessian_u_values[q][0][0][0] - prev_timestep_hessian_u_values[q][0][0][0]) + (lambda_values[q] + mu_values[q]) * (hessian_u_values[q][1][0][1] - prev_timestep_hessian_u_values[q][1][0][1]) + mu_values[q] * (hessian_u_values[q][0][1][1] - prev_timestep_hessian_u_values[q][0][1][1]) - biot_alpha * (grad_p_values[q][0] - prev_timestep_grad_p_values[q][0]);
